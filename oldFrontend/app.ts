...
const gameEnder = setInterval((): void => {
  if (isRunning && countDown > 0) {
    ...
    timerEl.innerHTML = `Time: ${Math.abs(Math.round(countDown * 10) / 10).toFixed(1)}`;
  }
  if (countDown <= 0) {
    isRunning = false;
...
function shot({ click, element, targetX, targetY, elementSize }: Shot): void {
  ...
  const distance: number = Math.sqrt(
    (click.x - targetCenterX) ** 2 + (click.y - targetCenterY) ** 2,
  );
  let score: number =
...
let stats = (totalScore: number): void => {
  ...
  shotsPerSecondEL.innerHTML = `Shots per second ${shotsPS.toFixed(2)}`;
  avgScorePerTargetEl.innerHTML = `Avg score per target: ${avg.toFixed(2)}`;
};
